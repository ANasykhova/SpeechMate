  ...
  const handleCreate = async () => {
    ...
    try {
      const requestParams = new URLSearchParams({
        ...
        voice_ids: createdProject.targetVoice.toString(),
        is_cloning: '0',
        num_speakers: numberOfSpeakersIsEmpty ? '' : newProject.numberOfSpeakers,
      });

      const pipelineUrl = 'http://0.0.0.0:8081' // OUR_PIPELINE_URL;
...
const VoiceItem: FC<VoiceItemProps> = (props) => {
  ...
  return (
    <div key={voice.voice_id} className="flex flex-row justify-start items-center">
      ...
      >
        <div className="flex flex-row w-full box-content justify-start items-center gap-5">
