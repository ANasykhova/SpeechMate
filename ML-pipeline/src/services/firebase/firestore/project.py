from configs.firebase import PROJECTS_COLLECTION
...
def update_project_status_and_translated_link_by_id(
    ...
    project_ref = firestore.collection(PROJECTS_COLLECTION).document(project_id)
    project_snap = project_ref.get()

    if not project_snap.exists:
        catch_error(
            tag=log_tag,
            error=Exception(f"Project with id {project_id} does not exist.")
        )
    ...
    if show_logs:
        print_info_log(
            tag=log_tag,
            message=f"Project was updated with fields: {project_fields_to_update}"
        )
